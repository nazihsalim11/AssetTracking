...
// The matrix cache, loadRolePermissions(), and roleAllows() now live in
// src/middleware/auth.js and are bound into locals above via createAuth().
...
app.patch('/api/role-permissions', async (req, res) => {
  ...
  try {
    ...
    auth.invalidateRolePermissions(); // force a fresh read on the next enforcement check
